...
Conventions used so far: `YYYY-MM-DD-short-slug.jpg` filenames, images downscaled to
1280px on the long edge (plenty for the full-size lightbox, and it keeps the whole
gallery around 2MB instead of 35MB of camera originals), metadata stripped in the
process, and newest first in `manifest.json` — the same order "My photos" uses.
